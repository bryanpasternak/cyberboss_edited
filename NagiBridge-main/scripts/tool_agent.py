...
def execute_tool(name, arguments):
    if name == "get_state":
        ...
        script_path = os.path.join(SCRIPT_DIR, "{}.py".format(script))
        if not os.path.exists(script_path):
            return {"error": "Script not found: {}.py".format(script)}
        cmd = "python \"{}\" {} --port {}".format(script_path, arguments.get('args', ''), args.port)
        try:
